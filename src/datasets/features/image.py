...
class Image:
    ...
    def cast_storage(self, storage: Union[pa.StringArray, pa.StructArray, pa.ListArray]) -> pa.StructArray:
        ...
        if pa.types.is_string(storage.type):
            ...
            # Iterate scalar-by-scalar so each row's raw array is encoded and released
            # before the next one is materialized, rather than keeping the whole
            # dataset in Python memory via storage.to_pylist().
            bytes_array = pa.array(
                (
                    encode_np_array(np.array(value))["bytes"] if value is not None else None
                    for value in (scalar.as_py() for scalar in storage)
                ),
                type=pa.binary(),
                size=len(storage),
            )
    ...
    def embed_storage(self, storage: pa.StructArray, token_per_repo_id=None) -> pa.StructArray:
        ...
        bytes_field = storage.field("bytes")
        path_field = storage.field("path")

        # Fast path: if every row already has bytes, reuse the Arrow buffer rather
        # than materializing multi-GB payloads into Python just to re-wrap them.
        if bytes_field.null_count == 0:
            bytes_array = array_cast(bytes_field, pa.binary())
        else:
            # Fetch only rows that need path->bytes; non-null rows stay in Arrow and
            # are merged via coalesce, skipping the Python round-trip for their bytes.
            paths = path_field.to_pylist()
            null_mask = bytes_field.is_null().to_pylist()
            fetched = [path_to_bytes(paths[i]) if null_mask[i] else None for i in range(len(storage))]
            fetched_array = pa.array(fetched, type=pa.binary())
            bytes_array = pc.coalesce(array_cast(bytes_field, pa.binary()), fetched_array)

        path_array = pa.array(
            [os.path.basename(path) if path is not None else None for path in path_field.to_pylist()],
            type=pa.string(),
        )
